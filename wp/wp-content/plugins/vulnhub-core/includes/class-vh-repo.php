...
final class Repo {
	...
	private static function normalise_names( array $data ): array {
		 ...
		 * workgroup name. The CMDB sends `workgroup\appsrv07` in its DNS
	...
	private static function known_domains(): array {
		...
		// Longest first, so `deep.sub.domain.example.com` is tried before
		// `example.com` and the short name comes out whole.
	 ...
	 * `appsrv01-corp-example`, which matched nothing and became five
	 ...
	 * `cloud-syslog-01` is never mistaken for one.
	 *
	 ...
	 *     three, so `cloud-index-ser` (Tenable, truncated to the NetBIOS
	 *     limit of 15) and `cloud-index-server` (the CMDB) meet on the FQDN
	 *     they share;
	...
	public static function match_asset( array $data ): ?array {
		...
		/*
		 * A NetBIOS name is capped at 15 characters, so `cloud-print-app`
		 * and `cloud-print-app-01` can be one machine. Corroborated by the
		 * address, because on its own a 15-character prefix would also
		 * match `appsrv09-legacy`, which is a different server on a
		 * different address.
		 */
	 ...
	 * limit of 15 (`cloud-index-ser`), Defender flattens the FQDN's dots
	 * (`appsrv01-corp-example`), and AWS answers with the address it
	...
	public static function upsert_asset( array $data ): array {
		 ...
		 * on it.
		 *
		 * Tenable reports `mgtsrv01.corp.example` in its hostname field and
		 * the CMDB reports `mgtsrv01`. Stored as they arrive, those are two
		 ...
		 * else, wrote a second `wkstnxj5albjqmd` beside the one Intune and the
	 ...
	 * `wkstnxj5albjqmd` came to read "Not in Tenable" on the row the CMDB,
	 ...
	 * because Tenable reports `host.corp.example` where every other feed
	...
	public static function duplicate_candidates(): array {
		...
		$queries = array(
			...
			// `cloud-index-ser` and the CMDB's `cloud-index-server` meet.
	 ...
	 * begins `wkstn` or `cloud-` -- and testing 232,441 findings against a
