...
Admin: `romy` — password in `.admin_pass` (chmod 600, alongside `.db_pass` and
`.vh_enc_key`).
...
## Publishing at vul.romynz.com

The app is already correct behind the tunnel — verified by replaying a tunnel
request (`Host: vul.romynz.com`, `X-Forwarded-Proto: https`): HTTP 200, all 18
absolute URLs emitted as `https://vul.romynz.com`, zero `localhost` leakage, no
...
> **Subdomain** `vul` · **Domain** `romynz.com` · **Service** `HTTP://localhost:8093`

The tunnel already serves `wpmcp.romynz.com`, so the zone is attached and DNS
