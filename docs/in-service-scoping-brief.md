...
- Root: `/srv/vulnhub` (docker compose: `vulnhub-wp`, `vulnhub-db`, `vulnhub-wpcli`)
- Plugins: `/srv/vulnhub/wp/wp-content/plugins/vulnhub-*`
- Live: at whatever hostname you've published it under (default: http://localhost:8093/)
...
Worse, it is internally inconsistent: the DNZ Bluecurrent OSG team card shows **ASSETS 655** (in-service
...
- Most exposed asset `nzaklsat1rh1.amssmp.local` (asset id 691, 6,635 open findings) is `lifecycle=unknown`,
